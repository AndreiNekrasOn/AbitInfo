...
from .models import Olymp, UniverPlus, Subject
...
def search(request):
    ...
    if 'olymp_name' in request.GET and request.GET['olymp_name']:
        ...
        univers = UniverPlus.objects.all().order_by('rating', 'title').filter(olymps__title=olymp_name)

        content = {'olymp_name': olymp_name, #'subjects': subjects,
                   'univers': univers, 'olymps': olymp_name}
        return render_to_response('olcalc/index.html', content)
    else:
        return render(request, 'olcalc/index.html', {#'subjects': subjects,
                                                     'olymps': olymps})
